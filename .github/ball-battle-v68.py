...
def rep(old, new, count=1):
    ...
    if found != count:
        raise SystemExit(f'expected {count} occurrences, found {found}: {old[:140]!r}')
...
rep('spawnFloatingText(f.x, f.y - f.r - 46, "약화된 리들 경 부활", "#e5e7eb");',
    'spawnFloatingText(f.x, f.y - f.r - 46, `약화된 리들 경 부활 · HP ${f.hp}`, "#e5e7eb");')
rep('log(`<strong>${escapeHtml(f.name)}</strong> 호크룩스 생존 · 체력 ${RIDDLE.weakHp}의 약화 상태로 부활`, "부활");',
    'log(`<strong>${escapeHtml(f.name)}</strong> 호크룩스 생존 · HP ${f.hp}/${RIDDLE.weakHp} 약화 상태로 부활 · 딱총나무 지팡이 사용 불가`, "부활");')

rep('if (!r.wand) return;', 'if (!r.wand || r.weakened) return;')
...
rep(
'첫 사망 시 7칸 호크룩스로 7초를 버티면 약화 상태로 부활한다.',
'첫 사망 시 7칸 호크룩스로 7초를 버티면 약화 상태로 부활하며, 대기 중 피격·충돌마다 부활 HP가 10씩 감소한다. 약화 부활 후에는 딱총나무 지팡이를 얻을 수 없다.'
)

rep(
'생존하면 HP90·이속2.5로 부활하며 피엔드피레 반경-15, 아바다 피해20% 감소. 두 번째 부활은 없다.',
'피격 또는 접촉 1회마다 부활 HP도 70에서 10씩 감소한다. 생존하면 최대HP70·이속2.5의 약화형으로 남은 부활HP만큼 부활하며 딱총나무 지팡이를 더 이상 획득할 수 없다. 피엔드피레 반경-15, 아바다 피해20% 감소. 두 번째 부활은 없다.'
)
